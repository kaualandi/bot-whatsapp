...
import { alterClient, getClients, pushByName } from "../service/query";
...
async function nameQuery(client: Client, message: Message) {
  ...
  if (data.error)
    return client.reply(
      from,
      `Não consegui recuperar os dados do nome: ${name}!\n${data.message}`,
      id
    );
  console.log(typeof data?.header?.error);
